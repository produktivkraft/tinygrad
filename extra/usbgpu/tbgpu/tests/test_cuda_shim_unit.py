...
class TestCUDAShimHelpers(unittest.TestCase):
  ...
  def test_build_launch_cbuf0_injects_dims(self):
    fake_prg = type("FakeProgram", (), {"cbuf_0": [0xDEAD_BEEF] * 8})()
    blob = cuda_shim._build_launch_cbuf0(fake_prg, (7, 2, 3), (64, 4, 5))
    words = list(memoryview(blob).cast("I"))
    self.assertEqual(words[:6], [64, 4, 5, 7, 2, 3])
    self.assertEqual(words[6:], [0xDEAD_BEEF, 0xDEAD_BEEF])

  def test_build_launch_cbuf0_empty(self):
    fake_prg = type("FakeProgram", (), {"cbuf_0": []})()
    self.assertEqual(cuda_shim._build_launch_cbuf0(fake_prg, (1, 1, 1), (1, 1, 1)), b"")
